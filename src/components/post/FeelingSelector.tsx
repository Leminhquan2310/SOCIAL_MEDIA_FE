  ...
  return (
    <div className="relative">
      <div
        ...
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-colors cursor-pointer select-none ${
          selected ? "bg-yellow-50 text-yellow-700" : "text-gray-500 hover:bg-gray-50"
        }`}
      >
        <Smile size={17} className={selected ? "text-yellow-600" : "text-yellow-500"} />
        <span className="text-[13px] font-semibold">
          {selected ? `Đang cảm thấy ${currentFeeling?.emoji} ${selected}` : "Cảm xúc"}
        </span>
        {selected && (
          <button
            type="button"
              ...
              {FEELINGS.map((feeling) => (
                <button
                  ...
                  className={`flex items-center gap-2 w-full px-3 py-2 rounded-lg text-left transition-colors ${
                    selected === feeling.label ? "bg-yellow-50 text-yellow-700" : "hover:bg-gray-50 text-gray-700"
                  }`}
                >
